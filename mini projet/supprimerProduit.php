...
<div class="container">
 ...
 <form action="" method="post"> 
     <br><br> <input type="text" name="sup" id="" placeholder="Nom du produit"> 
   
    <input type="submit" value="validez pour supprimer " class="login">
    </form>
    <br> <br>
    <?php 
...
{
    if (strcasecmp($_POST['sup'],$produits[$i][0])==0) {
        unset( $produits[$i]);
    }
}
...
for($i=0;$i<$lignes;$i++){
    echo '<tr>';
    if($produits[$i][1]<10 && $produits[$i][0]!=""){
        for($j=0;$j<$colonne;$j++){
            echo   '<th class="p-3 mb-2 bg-danger text-white">'.$produits[$i][$j].'</th>';
        }
    }
    elseif($produits[$i][1]>=10 && $produits[$i][0]!=""){
        for($j=0;$j<$colonne;$j++){
